...
const buildPartnerRouter = () => {
  ...
  partnerRouter.post(
    ...
    async (request: PartnerRequest, response: Response) => {
      try {
        const context = request.partnerContext!;
        const payload = request.body as CandidateImportRequest;
        const batch = await partnerDataStore.createCandidateBatch(
          context.credential.partnerId,
          context.environment,
          payload
        );
  ...
  partnerRouter.get('/candidates/import/:batchId', async (request: PartnerRequest, response: Response) => {
    const context = request.partnerContext!;
    const batchId = request.params.batchId;
    const record = await partnerDataStore.getCandidateBatch(
      context.credential.partnerId,
      context.environment,
      batchId
    );
  ...
  partnerRouter.post(
    ...
    async (request: PartnerRequest, response: Response) => {
      try {
        ...
        const record = await partnerDataStore.createAssessment(
          context.credential.partnerId,
          context.environment,
          payload
        );
  ...
  partnerRouter.post(
    ...
    async (request: PartnerRequest, response: Response) => {
      try {
        ...
        const record = await partnerDataStore.createBatchAssessment(
          context.credential.partnerId,
          context.environment,
          payload
        );
  ...
  partnerRouter.post(
    ...
    async (request: PartnerRequest, response: Response) => {
      try {
        ...
        const record = await partnerDataStore.recordPlacement(
          context.credential.partnerId,
          context.environment,
          payload
        );
...
export const createApp = () => {
  ...
  app.use('/api/v1', buildPartnerRouter());
